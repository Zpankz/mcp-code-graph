...
export default function createServer({ config: userConfig } = {}) {
    ...
    if (userConfig) {
        ...
        if (userConfig.orgId)
            config.CODEGPT_ORG_ID = userConfig.orgId;
        if (userConfig.graphId)
            config.CODEGPT_GRAPH_ID = userConfig.graphId;
        if (userConfig.repoUrl)
            config.CODEGPT_REPO_URL = userConfig.repoUrl;
    }
...
async function startHttpServer() {
    ...
    app.all('/mcp', async (req, res) => {
        ...
        if (req.query['config.apiKey']) {
            config.CODEGPT_API_KEY = req.query['config.apiKey'];
        }
        if (req.query['config.orgId']) {
            config.CODEGPT_ORG_ID = req.query['config.orgId'];
        }
        if (req.query['config.graphId']) {
            config.CODEGPT_GRAPH_ID = req.query['config.graphId'];
        }
        if (req.query['config.repoUrl']) {
            config.CODEGPT_REPO_URL = req.query['config.repoUrl'];
        }
